...
fn criterion_benchmark(c: &mut Criterion) {
    c.bench_function("natural_cmp random", |b| b.iter(|| {
    ...
    c.bench_function("natural_cmp_old random", |b| b.iter(|| {
        let mut rng = ChaCha20Rng::seed_from_u64(42);
        for _ in 0..1000 {
            let a = generate_string(&mut rng);
            let b = generate_string(&mut rng);
            black_box(natural_cmp_old(black_box(a.as_str()), black_box(b.as_str())));
        }
    }));
}
